                    ...
                    <form action="{{ route('tickets.update', $ticket) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="CodigoReserva" id="CodigoReserva" value="{{ old('CodigoReserva', $ticket->CodigoReserva) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>

                            <!-- Precio -->
                            <div class="mb-4">
                                <label for="precio" class="block text-sm font-medium text-gray-700">Precio</label>
                                <input type="number" name="precio" id="precio" value="{{ old('precio', $ticket->precio) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="usuario_id" id="usuario_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                                    @foreach ($usuarios as $usuario)
                                        <option value="{{ $usuario->id }}" {{ $ticket->usuario_id == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="autobus_id" id="autobus_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                                    @foreach ($autobuses as $autobus)
                                        <option value="{{ $autobus->id }}" {{ $ticket->autobus_id == $autobus->id ? 'selected' : '' }}>{{ $autobus->placa }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="sucursal_id" id="sucursal_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                                    @foreach ($sucursales as $sucursal)
                                        <option value="{{ $sucursal->id }}" {{ $ticket->sucursal_id == $sucursal->id ? 'selected' : '' }}>{{ $sucursal->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="horario_id" id="horario_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                                    @foreach ($horarios as $horario)
                                        <option value="{{ $horario->id }}" {{ $ticket->horario_id == $horario->id ? 'selected' : '' }}>{{ $horario->HoraSalida }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="ruta_id" id="ruta_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                                    @foreach ($rutas as $ruta)
                                        <option value="{{ $ruta->id }}" {{ $ticket->ruta_id == $ruta->id ? 'selected' : '' }}>{{ $ruta->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
